...
def viz_calibration(pts3d, grid, rvecs, tvecs, *, camera_idxs=None, axis_length=0.1, text_size=12):
    ...
    for i, (rvec, tvec) in enumerate(zip(rvecs, tvecs)):
        ...
        ax.scatter(cam_pos[0], cam_pos[1], cam_pos[2], c='black', marker='o', label="Camera centers" if i == 0 else "")
        
        # Visualize the camera orientation as an axis (draw lines for the camera orientation axes)
        xyz_axes = cam_rot.T*axis_length + cam_pos # Axes - vectors along the camera's x, y, z
        for axis, label, color in zip(xyz_axes, ['X Right','Y Down','Z Forward'], ['r', 'g', 'b']):
            ax.plot([cam_pos[0], axis[0]], [cam_pos[1], axis[1]], [cam_pos[2], axis[2]], c=color, label=label if i == 0 else None)
        ...
        ax.text(cam_pos[0], cam_pos[1], cam_pos[2],  str(camera_idxs[i]), size=text_size, zorder=1, color='k') 
