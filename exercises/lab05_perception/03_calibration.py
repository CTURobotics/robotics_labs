...
def calibrate(dir, grid, square_size, img_max_width=1024, viz=False):
    ...
    corners3d *= square_size # needed for extrinsics only
    ...
    err, K, dist, rvecs, tvecs = cv2.calibrateCamera(pts3d, pts2d, gray.shape[::-1], None, None)
    np.savez("calibration.npz", K=K, dist=dist, rvecs=rvecs, tvecs=tvecs, max_width=img_max_width)

    print(f"RMS re-projection error: {err}\n",)
    print(f"Camera matrix:\n{K}\n")
    print(f"Distortion coefficients:\n{dist}\n")
    
    if viz:
        viz_calibration(corners3d, grid, rvecs, tvecs,
...
if __name__ == "__main__":
    ...
    parser.add_argument("-d", "--dir", default="chessboard", type=str, help="Directory containing images")
    parser.add_argument("-w", "--width", type=int, default=1024, help="Maximum width of images (preprocessing)")
    parser.add_argument("-g", "--grid", type=str, default="6x9", help='Grid size (e.g. "5x7" points for a 4x6 chessboard)')
    parser.add_argument("-s", "--square-size", type=float, default=30.0, help="Square size")
    ...
    calibrate(args.dir, grid, args.square_size, img_max_width=args.width, viz=not args.no_show)
